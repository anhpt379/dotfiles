function fzf_find -d "Find files and folders"
    ...
    set -l result (printf "%s\n" $fd_results | devicon-lookup | fzf --ansi +m -1 --tiebreak=$tiebreak --expect=enter --header=":: Press TAB to accept suggestion, ENTER to accept suggestion and run." --bind=tab:accept --preview="~/.config/fzf/preview.sh {}" --query "$fzf_query")
