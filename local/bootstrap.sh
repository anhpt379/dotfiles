...
# fzf
cd /tmp || exit 1
curl -fLo ~/.local/share/man/man1/fzf.1 https://raw.githubusercontent.com/junegunn/fzf/0.41.1/man/man1/fzf.1
wget https://github.com/junegunn/fzf/releases/download/0.41.1/fzf-0.41.1-linux_arm64.tar.gz
tar zxvf fzf-*.tar.gz
mv fzf ~/.local/bin/
rm -rf fzf-*.tar.gz
cd - || exit 1
...
# lf
cd /tmp || exit
curl -fLo ~/.local/share/man/man1/lf.1 https://raw.githubusercontent.com/gokcehan/lf/r28/lf.1
wget https://github.com/gokcehan/lf/releases/download/r28/lf-linux-arm64.tar.gz
tar zxvf lf-*.tar.gz
mv lf ~/.local/bin/
rm -f lf-*.tar.gz
cd - || exit 1
...
# # echo "0 3 * * * bash -c \"curl -fsH 'Accept-encoding: gzip' https://<DOMAIN>/servers | gunzip - | jq -r '.servers | .[] | .fqdn' | sort | grep -v 'spare-' > ~/.cache/servers.tmp; if test -s ~/.cache/servers.tmp; then mv ~/.cache/servers.tmp ~/.cache/servers.txt; fi\" >>/tmp/cron.log 2>&1" >> cron
# crontab cron
# rm -f cron
